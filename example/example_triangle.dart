...
late Pointer<VkPhysicalDevice> physicalDevice = nullptr;
late int physicalDeviceCount = 0;
late int familyCount = 0;
late int graphicsFamily = -1;
late int presentFamily = -1;
...
void createSwapChain() {  
  ...
  createInfo.ref
    ...
    ..imageExtent_width = extent.width
    ..imageExtent_height = extent.height
    ..imageArrayLayers = 1
...
void createGraphicsPipeline() {
  final vertShaderCode = File('./example/main.vert.sprv').readAsBytesSync();
  final fragShaderCode = File('./example/main.frag.sprv').readAsBytesSync();
  ...
  final vertShaderStageInfo = shaderStages.elementAt(0);
...
void draw() {
  ...
  final waitStages = calloc<Int32>();
  waitStages.value = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
  submitInfo.ref
    ..waitSemaphoreCount = 1
    ..pWaitSemaphores = imageAvailableSemaphores.elementAt(currentFrame)
...
void createShaderModule(Pointer<Pointer<VkShaderModule>> shaderModule, List<int> code) {
  ...
  createInfo.ref
    ...
    ..pCode = NativeBuffer.fromBytes(code);
